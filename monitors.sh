...
#   monitors.sh patch …     replace-or-append hl.monitor lines in the block
...
#
# Applies run through BOTH channels: the panel evals for instant feedback
# AND calls `patch` to write the block — on O4 the config engine can revert
# a bare eval within ~800ms, while the config file is authoritative.
...
# patch "<hl.monitor line>" [...] — replace-or-append each given line in the
# marker block, keyed by its output name. This is the RELIABLE apply path on
# O4: hl.monitor via `hyprctl eval` registers a rule the config engine can
# re-assert right over (scale changes were seen reverting within ~800ms),
# while monitors.lua is authoritative and live-reloads on write. The panel
# runs both: eval for instant feedback, patch so the change actually holds.
patch)
    shift
    [ $# -ge 1 ] || { echo "usage: monitors.sh patch '<hl.monitor line>' ..." >&2; exit 2; }
    mkdir -p "$(dirname "$CONFIG")" || exit 1
    [ -f "$CONFIG" ] && cp "$CONFIG" "$BACKUP"
    # Existing block lines, minus any output we are about to rewrite.
    keep=""
    if [ -f "$CONFIG" ]; then
        keep=$(sed -n "/^-- >>> nosignal\.monitor-settings begin/,/^-- <<< nosignal\.monitor-settings end/p" "$CONFIG" \
            | grep '^hl\.monitor' | while IFS= read -r old; do
                out=$(printf '%s' "$old" | sed -n 's/.*output = "\([^"]*\)".*/\1/p')
                [ -n "$out" ] || continue
                skip=0
                for new in "$@"; do
                    nout=$(printf '%s' "$new" | sed -n 's/.*output = "\([^"]*\)".*/\1/p')
                    [ "$out" = "$nout" ] && { skip=1; break; }
                done
                [ "$skip" = 1 ] || printf '%s\n' "$old"
            done)
    fi
...
*)
    echo "usage: monitors.sh state|brightness|save|patch|restore|takeover" >&2
    exit 2
    ;;
esac
